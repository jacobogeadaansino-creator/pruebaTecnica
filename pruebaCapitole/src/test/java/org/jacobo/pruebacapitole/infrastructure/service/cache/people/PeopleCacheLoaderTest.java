...
class PeopleCacheLoaderTest {
    ...
    @BeforeEach
    void setUp() {
        loader = new PeopleCacheLoader(peopleSwapiPort, peopleCache, executor, peopleIdCache);
    }

    @Test
    void loadCache_firstPageNullOrEmpty() {
        when(peopleSwapiPort.getPeople()).thenReturn(null);
        loader.loadCache();
    ...
    void loadCache_firstPageEmptyResults() {
        ...
        when(peopleSwapiPort.getPeople()).thenReturn(dom);
        loader.loadCache();
        verify(peopleCache, never()).put(anyString(), any());
        verify(peopleIdCache, never()).populateCache();
    }
    ...
    void loadCache_fullFlow() throws InterruptedException {
        ...
        when(peopleSwapiPort.getPeople()).thenReturn(firstPage);
        ...
        when(peopleSwapiPort.getPeopleByPage(anyInt())).thenReturn(secondPage);
        // Executor real
        java.util.concurrent.ExecutorService realExecutor = java.util.concurrent.Executors.newFixedThreadPool(2);
        loader = new PeopleCacheLoader(peopleSwapiPort, peopleCache, realExecutor, peopleIdCache);
        loader.loadCache();

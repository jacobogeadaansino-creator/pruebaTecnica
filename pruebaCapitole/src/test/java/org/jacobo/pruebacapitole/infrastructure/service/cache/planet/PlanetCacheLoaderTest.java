...
class PlanetCacheLoaderTest {
    ...
    void loadCache_firstPageNullOrEmpty() {
        ...
        loader = new PlanetCacheLoader(planetSwapiService, planetCache, executor, planetIdCache);
        when(planetSwapiService.getPlanet()).thenReturn(null);
        loader.loadCache();
    ...
    void loadCache_firstPageEmptyResults() {
        ...
        when(planetSwapiService.getPlanet()).thenReturn(dom);
        loader.loadCache();
        verify(planetCache, never()).put(anyString(), any());
        verify(planetIdCache, never()).populateCache();
        executor.shutdownNow();
    ...
    void loadCache_fullFlow() throws InterruptedException {
        ...
        when(planetSwapiService.getPlanet()).thenReturn(firstPage);
        ...
        when(planetSwapiService.getPlanetByPage(anyInt())).thenReturn(secondPage);
